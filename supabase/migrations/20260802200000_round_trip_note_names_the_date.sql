-- "(details above)" makes a dispatcher scroll a message body to find out when
-- the return leg is. Name the date instead.
--
-- Also stamps `international` from the carrier at ingest rather than leaving it
-- on the column default. The departure airport is the better signal and
-- infer_international() prefers it, but nothing captures it at booking yet, so
-- null is passed and the carrier decides.
--
-- Whole body restated because the notes assembly and the insert both change.
